...
function renderRegister(role) {
  ...
  regForm.onsubmit = async e => {
    ...
    const data = db();
    const profile = {...fd};
    delete profile.user; delete profile.pass; delete profile.fotoFile; delete profile.recomendacionFiles;
    ...
    const photo = form.fotoFile?.files?.[0];
    if (photo) { profile.foto = filePath('img/profile', fd.user, photo); profile.fotoPreview = await fileAsDataUrl(photo); }
    const letters = [...(form.recomendacionFiles?.files || [])];
    if (letters.length) profile.recomendaciones = letters.map(file => ({ nombre: file.name, ruta: filePath('letterprofile', fd.user, file), tipo: file.type || 'archivo' }));
